...
export function DownloadSection({ releaseData }: Props = {}) {
  const { t } = useI18n();
  ...
  const config = releaseData ?? DOWNLOAD_CONFIG;
  const active = selected ?? "macos-arm64";
  const asset = config.downloads[active] as DownloadAsset | undefined;
  ...
  return (
    ...
    >
      ...
      >
        <div className="h-125 w-175 rounded-full bg-violet-600/8 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto max-w-3xl px-6 text-center">
        ...
        <div className="rounded-2xl border border-zinc-700 bg-zinc-900/80 backdrop-blur-sm p-8 mb-6 ring-1 ring-violet-500/10">
          ...
          {asset && (
            <>
              <a
                href={asset.url}
                className="group inline-flex items-center gap-3 rounded-xl bg-linear-to-r from-violet-600 to-indigo-600 px-8 py-4 text-base font-semibold text-white hover:from-violet-500 hover:to-indigo-500 transition-all shadow-2xl shadow-violet-500/20 hover:shadow-violet-500/35"
                rel="nofollow"
              >
                <Download className="h-5 w-5" />
                {interpolate(t.download.primaryCta, { platform: platformLabels[active] ?? active })}
                <span className="text-violet-200/60 text-sm font-normal">
                  {asset.size}
                </span>
              </a>
              <p className="mt-3 text-xs text-zinc-600">
                {interpolate(t.download.versionNote, { version: config.version, format: asset.format })}
              </p>
            </>
          )}
        </div>
        ...
        <div className="relative inline-block text-left">
          ...
          >
            {selected ? platformLabels[selected] : t.download.selectPlatform}
            <ChevronDown
